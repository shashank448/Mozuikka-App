...
class View:
    ...
    def setup_player(self):
        ...
        self.Button10.configure(command=self.delete_song_from_favourites)



    def delete_song_from_favourites(self):
        delfav_song_index_tuple = self.playList.curselection()
        try:
            if len(delfav_song_index_tuple) == 0:
                raise NoSongSelectedError("Please select a song to add into favourites")
            song_name = self.playList.get(delfav_song_index_tuple[0])
            print("Bhai")
    ...
    def load_previous_song(self):
        try:
            ...
            print("Previous song index",self.prev_song_index)
            self.play_song()
        except(NoSongSelectedError) as ex1:
            messagebox.showerror("Error..!",ex1)

    def load_next_song(self):
        self.next_song_index=self.playList.curselection()[0]
        if self.next_song_index==self.playList.count()-1:
            self.next_song_index=-1
        self.next_song_index=self.next_song_index+1
        print(self.next_song_index)
        self.playList.select_clear(0, tk.END)
    ...
    def play_song(self):
        ...
        try:
            ...
            if(self.isThreadRunning==False):
                ...
                print("self.isrunning ke ander value true set ki")
            else:
    ...
    def setup_thread(self):
        print("setup thread")
        self.my_thread=threading.Thread(target=self.showtimer,args=(self.song_length,))
        self.my_thread.start()
        print("thread started")

    def showtimer(self,total_sec):
        print("show timer")
        curr_sec=0
        prev_perc_incr=0
    ...
    def load_song_from_favourites(self):
        try:
            ...
            if result.find("No Song in the Favourites") != -1:
    ...
    def closewindow(self):
        ...
        result=messagebox.askyesno("Quitting","Are you sure ...?")
        if result==True:
